# import matplotlib.pyplot as plt # Dikomentari sesuai kebutuhan Anda
import numpy as np
import sys
import json
import base64

# Fungsi keanggotaan fuzzy (dari contoh kedua, diubah untuk return float eksplisit)
def fuzzy_low(value, min_val, max_val):
    if value <= min_val:
        return 1.0
    elif value >= max_val:
        return 0.0
    # Tambahan untuk menghindari pembagian dengan nol jika min_val == max_val
...
# Data dan Fungsi dari "KRITERIA NILAI JURUSAN SEPERTI INI" (Contoh Kode Kedua)

# Daftar jurusan dengan nama dan threshold skor fuzzy minimum untuk diterima
param_jurusan = {
...
# Kriteria mata pelajaran untuk setiap jurusan
subject_criteria = {
    "TKJ": {
    ...
    "PH": {
        ...
        "non_unggulan": {"Matematika": 70, "Bahasa Indonesia": 72, "Fisika": 65, "Kimia": 65, "Biologi": 66, "PPKn": 69, "Sosiologi": 68, "Prakarya": 67, "Informatika": 60} # PH ada informatika di non-unggulan
    },
    "TBG": {
    ...
    "TBS": {
        ...
        "non_unggulan": {"Bahasa Inggris": 70, "Matematika": 68, "Fisika": 65, "Kimia": 65, "Biologi": 66, "PPKn": 69, "IPS": 68, "Sosiologi": 67, "Informatika": 60} # TBS ada informatika di non-unggulan
    },
    "FARM": {
        "unggulan": {"Biologi": 88, "Kimia": 85, "Matematika": 80},
        "non_unggulan": {"Bahasa Indonesia": 72, "Bahasa Inggris": 70, "Fisika": 68, "PPKn": 69, "IPS": 67, "Sosiologi": 66, "Prakarya": 65, "Seni Budaya": 65, "Informatika": 60} # FARM ada informatika di non-unggulan
    }
}
# Perbaikan subject_criteria dari contoh kedua Anda, beberapa jurusan tidak memiliki Informatika
# Pastikan semua mapel di subject_criteria ada di student_scores atau ditangani dengan .get(mapel, 0)

# Fungsi untuk mengambil range fuzzy sesuai jurusan
...
def calculate_academic_score(student_scores, kriteria_jurusan_mapel):
    ...
    if total_mapel_unggulan > 0: # Hanya proses jika ada mapel unggulan didefinisikan
        ...
        if jumlah_unggulan_terpenuhi == total_mapel_unggulan:
            ...
            return 0 # Jika mapel unggulan ada dan tidak semua terpenuhi, skor akademik 0
    else: # Jika tidak ada mapel unggulan yang didefinisikan, anggap syarat unggulan terpenuhi
        score_unggulan = 60


    score_non_unggulan = 0
    # Pastikan kriteria_jurusan_mapel memiliki key "non_unggulan"
    ...
    else: # Jika tidak ada mapel non-unggulan yang didefinisikan, dapat poin penuh untuk non-unggulan
        score_non_unggulan = 40
        
    return score_unggulan + score_non_unggulan

# Modifikasi hitung_tsukamoto dari contoh kedua (parameter jurusan_kode_pilihan dihilangkan karena tidak dipakai)
def hitung_tsukamoto(student_all_scores, tinggi, berat):
    ...
    for kode_jur, jur_info in param_jurusan.items():
        ...
        # kriteria_mapel_jurusan diambil dari subject_criteria global
        kriteria_mapel_jurusan = subject_criteria.get(kode_jur, {"unggulan": {}, "non_unggulan": {}}) # Ambil kriteria, default ke kosong jika tidak ada
        
        akademik_score_val = calculate_academic_score(student_all_scores, kriteria_mapel_jurusan)

        # Fungsi helper untuk fuzzifikasi, bisa diletakkan di sini atau global jika tidak bergantung pada current_fuzzy_range secara langsung
        def calc_derajat(label_param_calc, value_param_calc, range_to_use): # Ubah nama parameter agar tidak clash
            # param_range_calc = sorted(current_fuzzy_range[label_param_calc]) # Ambil dari argumen range_to_use
        ...
        detail = {
            ...
            "rules": {} # Inisialisasi untuk detail rule
        }
        
        # Fuzzifikasi untuk setiap parameter
        ...
        # Aturan Fuzzy Tsukamoto dari contoh kedua
        rules_applied = [] # Ganti nama variabel agar tidak clash dengan 'rules' di detail
        ...
        z2 = 100 - 50 * alpha2 # Jika akademik sangat rendah (low=1), z2=50. Jika akademik tinggi (low=0), z2=100. Ini mungkin terbalik dari intensi rendah->rendah.
                               # Seharusnya jika akademik rendah (alpha2 tinggi), Z harus rendah. Misal: z2 = 50 - 40 * alpha2
        z2_corrected = 50 - 40 * alpha2 # Jika low=1 (sangat rendah), z2=10. Jika low=0 (tidak rendah), z2=50.
        ...
        z4 = 70 - 30 * alpha4 # Jika fisik kurang (alpha4 tinggi), Z harus rendah. Misal: z4 = 50 - 30 * alpha4
        z4_corrected = 50 - 30 * alpha4 # Jika low avg = 1 (sangat kurang), z4=20. Jika low avg = 0 (ideal), z4=50.
        ...
        final_score = max(0, min(final_score, 100)) # Pastikan skor 0-100
        ...
        hasil[kode_jur] = {
            ...
            "detail_hitungan": detail, # Berisi nilai input, range, fuzzifikasi, dan detail rule
            "range_fuzzy_jurusan_digunakan": current_fuzzy_range, # Untuk referensi range yang dipakai
            "kriteria_mapel_jurusan": kriteria_mapel_jurusan # Untuk referensi kriteria mapel yang dipakai
        }
    return hasil

# BLOK UTAMA UNTUK EKSEKUSI VIA COMMAND LINE (DARI CONTOH KODE PERTAMA)
    ...
    if len(sys.argv) > 1:
        try:
            ...
            try:
                ...
                # Validasi nilai mapel dalam student_scores
            ...
            # Menggunakan hitung_tsukamoto yang telah dimodifikasi dari contoh kedua
            ...
            # print(json.dumps({"error": f"Error during fuzzy calculation: {str(e)}", "trace": traceback.format_exc()}, ensure_ascii=False))
            print(json.dumps({"error": f"Error during fuzzy calculation: {str(e)}"}, ensure_ascii=False))
            sys.exit(1)
    else:
        # Info jika tidak ada argumen CLI
        # Untuk testing, Anda bisa hardcode input di sini atau memanggil fungsi dengan data contoh
        info_msg = {
