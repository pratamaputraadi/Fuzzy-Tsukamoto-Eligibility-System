...
$data_lengkap = true; // Asumsi awal data lengkap
...
if (isset($_GET['student_id']) && is_numeric($_GET['student_id'])) {
    ...
    try {
        ...
        // Periksa apakah data nilai (12 mapel) sudah ada
        ...
        if ($physical_count < 1) {
            $data_lengkap = false;
            // Gabungkan pesan jika nilai juga belum lengkap
        ...
        if (!$data_lengkap) {
             $_SESSION['form_message_type'] = "error";
             header("Location: input_student_data.php?student_id=" . $student_id); // Arahkan ke halaman input data
             exit;
        }
...
// Daftar jurusan (hardcode sesuai param_jurusan di Python untuk kemudahan)
// Sebaiknya ini sinkron dengan keys di param_jurusan Python Anda
$majors = [
...
<head>
    ...
    <style>
        ...
        .form-container button { width: 100%; padding: 12px 15px; background-color: #17a2b8; /* Teal */ color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 16px; font-weight: 600; margin-top: 10px; }
