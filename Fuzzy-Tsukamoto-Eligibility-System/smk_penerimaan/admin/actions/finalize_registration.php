...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['student_id']) && isset($_POST['chosen_major_code'])) {
    ...
    try {
        ...
        // 1. Ambil dan validasi data siswa
        ...
        // 2. Siapkan data untuk JSON, lalu encode ke JSON, lalu encode ke Base64
        $data_for_python = [
        ...
        // Encode JSON string ke Base64
        $base64_encoded_payload = base64_encode($input_json_for_python);

        // --- BLOK DEBUG JIKA MASIH PERLU (Nonaktifkan 'exit;' untuk menjalankan Python) ---
        /*
        echo "DEBUG MODE: finalize_registration.php<br><hr>";
        echo "Student ID: " . htmlspecialchars($student_id) . "<br>";
        echo "Chosen Major: " . htmlspecialchars($chosen_major_code) . "<br>";
        echo "Data Scores (raw): <pre>"; print_r($student_scores_from_db); echo "</pre>";
        echo "Height (raw): " . htmlspecialchars($student_height_from_db) . "<br>";
        echo "Weight (raw): " . htmlspecialchars($student_weight_from_db) . "<br>";
        echo "<hr>JSON String (sebelum Base64): <pre>" . htmlspecialchars($input_json_for_python) . "</pre>";
        echo "<hr>Base64 Encoded Payload (dikirim ke Python): <pre>" . htmlspecialchars($base64_encoded_payload) . "</pre>";
        // error_log("Base64 payload to Python: " . $base64_encoded_payload);
        // exit; // Hapus atau komentari baris ini untuk melanjutkan eksekusi Python
        */
        // --- AKHIR BLOK DEBUG ---

        $conn->beginTransaction(); // Mulai transaksi sekarang, sebelum panggil Python & ubah DB

        // 3. Jalankan skrip Python dengan payload Base64
        ...
        // 4. Tentukan status & rekomendasi (Sama seperti sebelumnya)
        ...
        if (isset($student_id) && is_numeric($student_id) && isset($conn)) {
            ...
            } catch (PDOException $ex_sname) { /* abaikan */ }
        }
