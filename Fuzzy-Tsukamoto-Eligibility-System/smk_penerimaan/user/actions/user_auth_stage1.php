...
function parse_birth_details($birth_details_str) {
    // Daftar bulan dalam bahasa Indonesia untuk mapping
    $bulan_map = [
    ...
    // Pecah berdasarkan koma pertama untuk memisahkan tempat dan tanggal
    ...
    // Pecah bagian tanggal (DD Bulan YYYY)
    $date_parts = explode(' ', $date_str);
    if (count($date_parts) !== 3) {
        return null; // Format tanggal tidak sesuai
    }

    $day = trim($date_parts[0]);
    $month_name = strtolower(trim($date_parts[1])); // ke huruf kecil untuk mapping
    ...
    // Format tanggal menjadi YYYY-MM-DD untuk perbandingan dengan database
    // Pastikan hari dan bulan memiliki dua digit
    ...
    if (!$d || $d->format('Y-m-d') !== $birth_date_db_format) {
        return null; // Tanggal tidak valid (misal: 30 Februari 2007)
    }
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        // error_log("User Login Stage 1 Error: " . $e->getMessage());
